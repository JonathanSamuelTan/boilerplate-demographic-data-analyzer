...
def calculate_demographic_data(print_data=True):
    # Read data from file
    df = pd.read_csv('my-project/adult.data.csv', delimiter = ',')
    ...
    bachelor = df['education'].value_counts().get('Bachelors',0)
    ...
    higher_education_rich = round((higher_education['salary'].value_counts().get('>50K',0)) / len(higher_education) * 100,1)
    lower_education_rich = round((lower_education['salary'].value_counts().get('>50K',0)) / len(lower_education) * 100,1)
    ...
    top_IN_occupation = df[(df['native-country'] == 'India') & (df['salary'] == '>50K')]['occupation'].value_counts().idxmax().round(1)
